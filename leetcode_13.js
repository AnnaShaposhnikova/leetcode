...
const romanToInt = function (s) {
   ...
   let arrFromS = s.split("");
    ...
    while (arrFromS.length){
        if(Object.keys(doubleobj).includes(arrFromS[0]+arrFromS[1])){
            const doubleDigit = arrFromS[0]+arrFromS[1];
            result += doubleobj[doubleDigit];
            arrFromS = arrFromS.slice(2);
        }else{
            result += convertObj[arrFromS[0]];
            arrFromS = arrFromS.slice(1);
        }
